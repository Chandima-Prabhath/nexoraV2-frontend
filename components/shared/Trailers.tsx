...
const TrailersComp: React.FC<TrailersProps> = ({ trailers }) => {
    ...
    if (!trailers || trailers.length === 0) {
        return <p data-oid="ilbydhy">No trailers available.</p>;
    }

    const currentTrailer = trailers[currentIndex];
    const match = currentTrailer.url.match(regex);
    const videoId = match ? match[1] : null;
    if (!videoId) return <p data-oid="269kg54">Invalid trailer URL.</p>;
    ...
    return (
        <div className="relative h-60 w-full rounded-md mb-4" data-oid="8_ame32">
            <iframe
                className="h-full"
